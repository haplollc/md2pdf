...
struct md2pdfTests {
    ...
    /// Code blocks and blockquotes used to share the docC `blockquote`
    /// styling (rounded aside-box), so the closing `> Generated by …`
    /// quote and adjacent code fences looked identical. This test renders
    /// a doc with both and asserts that the bands of "code-block color"
    /// (light gray from the docC `codeBackground`) and "blockquote color"
    /// (aside-note background) appear on *different* rows of the page.
    @Test func codeBlockAndBlockquoteRenderDifferently() async throws {
        let vm = EditorViewModel()
        vm.markdownContent = """
        # Style separation
        ...
        // Sample one column near the page horizontal center. Group ink-band
        // pixels by approximate color to detect distinct background tones.
        let pw = bitmap.pixelsWide
        let ph = bitmap.pixelsHigh
        var distinctColors = Set<UInt32>()
        for y in 0..<ph {
            // Sample a few horizontal positions per row so we catch wide
            // bands that span the column width.
            for sx in [pw / 4, pw / 2, 3 * pw / 4] {
                guard let color = bitmap.colorAt(x: sx, y: y) else { continue }
                let r = color.redComponent, g = color.greenComponent, b = color.blueComponent
                // Skip pure-white background and near-black text.
                if min(r, g, b) > 0.96 || max(r, g, b) < 0.30 { continue }
                // Quantize to ~8 levels per channel so anti-aliased
                // gradients on the same band cluster into one bucket.
                let qr = UInt32(r * 7), qg = UInt32(g * 7), qb = UInt32(b * 7)
                distinctColors.insert((qr << 16) | (qg << 8) | qb)
            }
        }

        // The page should have at least two distinct background colors —
        // one from the code block, one from the blockquote aside.
        // (Plus possibly the page background, but that's near-white and
        // filtered out above.)
        #expect(distinctColors.count >= 2,
